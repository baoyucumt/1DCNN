...
INPUT_NODE = 70   #输入层的节点数，这里是图片的像素
OUTPUT_NODE = 2   # 输出层的节点数，这里是类别的数目
IMAGE_SIZE1 = 1   # 图片大小
IMAGE_SIZE2 = 70
NUM_CHANNELS = 1  # 数字通道
NUM_LABELS = 2  # 数字类别
CONV1_DEEP = 32  # 第一层卷积层的尺寸和深度
CONV1_SIZE1 = 1
CONV1_SIZE2 = 21
CONV2_DEEP = 64  # 第二层卷积层的尺寸和深度
CONV2_SIZE1 = 1
CONV2_SIZE2 = 5
...
class myLeNet5(nn.Module):
    def __init__(self):
        ...
        #最后是三个全连接层
        self.fc1 = nn.Linear(64*1*17,128) #64*1*17, 128)  # 这里把第三个卷积当作是全连接层了
...
class MyLeNet5addone(nn.Module):
    def __init__(self):
        '''构造函数，定义网络的结构'''
        super().__init__()
        #定义显示中间图像的位置
        ...
        self.conv2 = nn.Conv2d(CONV1_DEEP, CONV2_DEEP, (CONV2_SIZE1,CONV2_SIZE2), padding=(0,2))
        # 尝试增加一个卷积
        self.conv3 = nn.Conv2d(CONV2_DEEP, CONV3_DEEP, (CONV3_SIZE1, CONV3_SIZE2), padding=(0, 2))

        #最后是三个全连接层
        self.fc1 = nn.Linear(768,128) #64*1*17, 128)  # 这里把第三个卷积当作是全连接层了
        self.fc2 = nn.Linear(128, 64)
        self.fc3 = nn.Linear(64, 2)  #output is 2
    ...
    def forward(self, x):
        ...
        incov=self.conv1(x)
        if self.showlayer==1:
            self.featuremap1 = incov.detach()  # 抽取特征核心代码
        input=F.relu(incov)
        x = F.max_pool2d(input, (1, 2),stride=(1,2),padding=0)#,stride=(0, 2), padding=(0,1))
        if self.showlayer==2:
        ...
        if self.showlayer==3:
            self.featuremap1 = x.detach()  # 核心代码
        #第三次卷积+pool
        x = F.max_pool2d(F.relu(self.conv3(x)), (1, 2), stride=(1, 2), padding=(0, 0))
        if self.showlayer==4:
            self.featuremap1 = x.detach()  # 核心代码
        #重新塑形,将多维数据重新塑造为二维数据，40*400
        x = x.view(-1, self.num_flat_features(x))
